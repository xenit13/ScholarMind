...
def _build_candidate_extraction_prompt(round_messages: list[dict]) -> str:
    ...
    return (
        ...
        "or `explicit`.\n\n"
        "# Output schema (strict — wrong types will be rejected)\n"
        "Return valid JSON only with this exact top-level shape:\n"
        '{"candidates": [ <candidate>, ... ]}\n\n'
        "Each `<candidate>` object MUST use these field names, types, and ranges:\n"
        "- `memory_type` (string, required): MUST be one of the EXACT values "
        "`preference`, `research_interest`, `knowledge_level`, `goal`, `workflow`, "
        "`project_constraint`, `paper_read`, `interaction_summary`, `feedback`. "
        "Any other value (e.g. `structured`, `read_paper`, `memory`, `fact`) is INVALID.\n"
        "- `content` (string, required): the durable fact, written in third person. "
        "Non-empty.\n"
        "- `structured` (object, optional, default `{}`): `memory_fact_v1` payload per "
        "the Discrete fact storage section above. Omit if not applicable.\n"
        "- `keywords` (array of strings, optional, default `[]`): each entry MUST be a "
        "string. Do NOT pass a bare string.\n"
        "- `importance` (number, optional, default `0.6`): MUST be a float in `[0.0, 1.0]`. "
        "Words like `medium`/`high`/`中`/`高` are INVALID — convert them to numbers first "
        "(low≈0.3, medium≈0.5, high≈0.8).\n"
        "- `confidence` (number, optional, default `0.7`): MUST be a float in `[0.0, 1.0]`. "
        "Same rule as `importance`.\n"
        "- `source` (string, required): MUST be one of the EXACT values `explicit`, "
        "`conversation`, `system_extracted`. Any other value (e.g. `message-0`) is INVALID.\n"
        "- `evidence` (array of objects, optional, default `[]`): each entry MUST be an "
        "OBJECT, e.g. `{\"message_id\": \"message-0\"}`. A bare string like `\"message-0\"` "
        "is INVALID — wrap it as `[{\"message_id\": \"message-0\"}]`.\n\n"
        "# Worked example\n"
        "User said: 我把论文《SWE-chat》标为 anchor paper，默认阅读深度 implementation-first。\n\n"
        "Correct output (two candidates, every field has the right type):\n"
        "```json\n"
        "{\n"
        "  \"candidates\": [\n"
        "    {\n"
        "      \"memory_type\": \"paper_read\",\n"
        "      \"content\": \"User marked the paper 'SWE-chat' as an anchor paper.\",\n"
        "      \"structured\": {\n"
        "        \"schema_version\": \"memory_fact_v1\",\n"
        "        \"fact_kind\": \"discrete_fact\",\n"
        "        \"subject\": {\"type\": \"user\", \"id\": \"user-1\", \"label\": \"User\"},\n"
        "        \"entity\": {\"type\": \"paper\", \"id\": \"paper:swe-chat\", \"label\": \"SWE-chat\"},\n"
        "        \"attribute\": \"role\",\n"
        "        \"value\": {\"canonical\": \"anchor_paper\", \"text\": \"anchor paper\"},\n"
        "        \"polarity\": \"positive\",\n"
        "        \"certainty\": \"explicit\",\n"
        "        \"temporal\": {\"tense\": \"current\"},\n"
        "        \"conflict_key\": \"subject:user:user-1|entity:paper:swe-chat|attribute:role\",\n"
        "        \"source_mode\": \"conversation\"\n"
        "      },\n"
        "      \"keywords\": [\"SWE-chat\", \"anchor paper\"],\n"
        "      \"importance\": 0.8,\n"
        "      \"confidence\": 0.9,\n"
        "      \"source\": \"conversation\",\n"
        "      \"evidence\": [{\"message_id\": \"message-0\"}]\n"
        "    },\n"
        "    {\n"
        "      \"memory_type\": \"preference\",\n"
        "      \"content\": \"User's default reading depth is implementation-first.\",\n"
        "      \"structured\": {\n"
        "        \"schema_version\": \"memory_fact_v1\",\n"
        "        \"fact_kind\": \"discrete_fact\",\n"
        "        \"subject\": {\"type\": \"user\", \"id\": \"user-1\", \"label\": \"User\"},\n"
        "        \"entity\": {\"type\": \"task\", \"id\": \"task:paper-reading\", \"label\": \"paper reading\"},\n"
        "        \"attribute\": \"preference\",\n"
        "        \"value\": {\"canonical\": \"implementation_first\", \"text\": \"implementation-first\"},\n"
        "        \"polarity\": \"positive\",\n"
        "        \"certainty\": \"explicit\",\n"
        "        \"temporal\": {\"tense\": \"current\"},\n"
        "        \"conflict_key\": \"subject:user:user-1|entity:task:paper-reading|attribute:preference\",\n"
        "        \"source_mode\": \"conversation\"\n"
        "      },\n"
        "      \"keywords\": [\"default depth\", \"implementation-first\"],\n"
        "      \"importance\": 0.7,\n"
        "      \"confidence\": 0.9,\n"
        "      \"source\": \"conversation\",\n"
        "      \"evidence\": [{\"message_id\": \"message-0\"}]\n"
        ...
        "```\n\n"
        "# Final reminder\n"
        "- `memory_type`, `source` → use EXACT enum strings, never synonyms.\n"
        "- `importance`, `confidence` → numbers in `[0,1]`, never words.\n"
        "- `evidence`, `keywords` → arrays, never scalars.\n\n"
