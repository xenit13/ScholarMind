...
async def ainvoke_model(model: Any, prompt: Any):
    return await _ainvoke_model(model, prompt)


async def route_tool_calls(
    state: list[Any] | dict[str, Any] | Any,
    messages_key: str = "messages",
) -> Literal["tools", "__end__"]:
    # Keep the tool router async so async subgraphs do not hang on sync
    # conditional edge execution during astream/ainvoke teardown.
...
def recent_tool_interaction(messages: list[BaseMessage]) -> list[BaseMessage]:
    interaction: list[BaseMessage] = []
    for message in reversed(messages):
        ...
        if isinstance(message, AIMessage) and message.tool_calls:
            ...
            continue
        break
    return list(reversed(interaction))


def recent_tool_messages(messages: list[BaseMessage]) -> list[ToolMessage]:
    return [
        message
        for message in recent_tool_interaction(messages)
        if isinstance(message, ToolMessage)
    ]


def all_tool_messages(messages: list[BaseMessage]) -> list[ToolMessage]:
    return [message for message in messages if isinstance(message, ToolMessage)]


def new_messages_since(
    previous_messages: list[BaseMessage], current_messages: list[BaseMessage]
) -> list[BaseMessage]:
    baseline = len(previous_messages)
    if baseline <= 0:
        return list(current_messages)
    if len(current_messages) <= baseline:
        return []
    return list(current_messages[baseline:])


def make_tool_trace_message(name: str, payload: Any, tool_call_id: str | None = None) -> ToolMessage:
    return ToolMessage(
        content=json.dumps(payload, ensure_ascii=False),
        tool_call_id=tool_call_id or f"fallback-{name}",
        name=name,
    )


def rerank_retrieved_chunks(
    query: str, chunks: list[dict[str, Any]], limit: int = 4
) -> list[dict[str, Any]]:
    candidates = [chunk for chunk in chunks if chunk.get("section") != "metadata"] or chunks
    ranked = sorted(
        candidates,
        key=lambda chunk: overlap_score(query, chunk.get("content", "")),
        reverse=True,
    )
    return ranked[:limit]
...
def try_structured_output(
    ...
    result, _ = invoke_structured_output(llm, prompt, schema, recover=recover)
    return result
...
def _structured_output_runnable(llm, schema):
    ...
    if _accepts_keyword(structured_output, "method"):
        # DeepSeek thinking-mode rejects function_calling ("tool_choice unsupported")
        # and json_schema ("response_format type unavailable"). json_mode sends
        # response_format={"type":"json_object"} which works in thinking mode.
...
def invoke_text_output(llm, prompt: Any):
    text, usage, _ = invoke_text_output_with_raw(llm, prompt)
    return text, usage


async def ainvoke_text_output(llm, prompt: Any):
    text, usage, _ = await ainvoke_text_output_with_raw(llm, prompt)
    return text, usage


def invoke_text_output_with_raw(llm, prompt: Any):
    if llm is None:
        return None, empty_usage(), None
    started = perf_counter()
    try:
        ...
        logger.exception("Text output invocation failed")
        return None, empty_usage(), None
    content = getattr(result, "content", "")
    text = str(content).strip() or None
    usage = usage_from_result(prompt, text or "", perf_counter() - started, result)
    return text, usage, result


async def ainvoke_text_output_with_raw(llm, prompt: Any):
    if llm is None:
        return None, empty_usage(), None
    started = perf_counter()
    try:
        result = await _ainvoke_model(llm, prompt)
    except Exception:
        logger.exception("Text output invocation failed")
        return None, empty_usage(), None
    content = getattr(result, "content", "")
...
def invoke_text_output_with_fallback(llms: list[Any], prompt: Any):
    merged_usage = empty_usage()
    for llm in llms:
        if llm is None:
            continue
        text, usage = invoke_text_output(llm, prompt)
...
async def ainvoke_text_output_with_fallback(llms: list[Any], prompt: Any):
    merged_usage = empty_usage()
    for llm in llms:
        if llm is None:
            continue
        text, usage = await ainvoke_text_output(llm, prompt)
        merged_usage = merge_usage(merged_usage, usage)
        if text:
            return text, merged_usage
    return None, merged_usage
